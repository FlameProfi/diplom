...
namespace Backend.Controllers
{
    ...
    [ApiController]
    [Authorize(Roles = "ADMIN,MANAGER,WAREHOUSE_WORKER,LOGIST,ACCOUNTANT,CLIENT")]
    ...
    public class NotificationsController : ControllerBase
    {
        ...
        {
            var items = await _context.Notifications.ToListAsync();
            return Ok(items);
        }
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<ActionResult<Notification>> Update(string id, Notification item)
        {
            if (id != item.Id)
            {
