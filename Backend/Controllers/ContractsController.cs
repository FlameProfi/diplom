...
namespace Backend.Controllers
{
    ...
    public class ContractsController : ControllerBase
    {
        ...
        {
            var items = await _context.Contracts.ToListAsync();
            return Ok(items);
        }
        ...
        public async Task<ActionResult<Contract>> Create(Contract item)
        {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<ActionResult<Contract>> Update(string id, Contract item)
        {
            if (id != item.Id)
            {
